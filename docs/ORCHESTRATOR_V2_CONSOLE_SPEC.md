...
- model health is now operator-visible: planner model verification and Codex/executor model/config health are surfaced through explicit protocol actions and status snapshots; unavailable model errors become Action Required/What Happened/Live Output items without silent fallback
- run elapsed time is surfaced when durable timestamps are available
...
Codex readiness should also be visible to the operator. The shell may only show model, effort, sandbox, or full-access state when the engine can verify it. If not verifiable, it must say `Not verified` and provide a concrete check path such as model-test protocol actions or `orchestrator doctor`; it must not silently claim the strongest model or full access.

(function registerViewModel(root, factory) {
  ...
  function buildDebugBundleText(snapshot, artifactListing, events = [], options = {}) {
    ...
    const bundle = [
      ...
      `- Binary version: ${safeString(backend.binary_version || runtime.version || runtime.binary_version || runtime.build_version || updateStatus.current_version || (snapshot && snapshot.version), "Unavailable")}`,
      `- Backend protocol: ${safeString((snapshot && snapshot.protocol && snapshot.protocol.version) || backend.protocol_version || runtime.protocol_version, "Unavailable")}`,
      `- Runtime config support: ntfy=${(backend.supports_ntfy_runtime_config || (backend.supports && backend.supports.ntfy_runtime_config) || runtime.supports_ntfy_runtime_config || (snapshot && snapshot.protocol && snapshot.protocol.supports && snapshot.protocol.supports.ntfy_runtime_config)) ? "yes" : "no"}`,

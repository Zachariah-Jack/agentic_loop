...
- Upgraded Side Chat from recorded-only notes to a non-interfering context-agent foundation that answers from observable runtime state while preserving raw messages.
...
- Side Chat does not yet include the final LLM-backed tooling/escalation backend and does not alter active runs by default.
